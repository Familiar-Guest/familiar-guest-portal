...
- **Rental Income &amp; Tax Payment Accounting built in** — FG tracks rental income, calculates/collects lodging taxes, withholds &amp; remits Mexican host taxes where it is the platform-of-record, and gives owners clean accounting plus **documents for tax reporting** across all their units. A core differentiator no competitor offers. (Handling + reporting, NOT tax advice.)
...
- **Acquisition hook (REVISED June 2026): PAYG-first, "first month commission-free," no card to start.** This is the primary launch offer and the front door for new owners. (Supersedes the briefly-adopted "first booking free" hook — reverted to the month-based offer; the barrier for this audience is still *risk and effort, not price*, but the offer is framed as a free trial window rather than a single free transaction.) Specifics:
  - **Sign up free — no monthly fee, no card required.** Removes commitment/setup risk; nothing to lose by setting up and testing.
  - **First month: 0% Familiar Guest commission** (owner pays only the pass-through payment processing). For the full first month the owner experiences the core value — escrow, payout, Airbnb fee gone, guest relationship kept — at zero FG commission. This is *forgone* revenue, not a cash outlay, and is a cheap CAC.
  - **After the first month:** 5% PAYG, or the owner switches to a flat plan when the math flips (surface the crossover automatically).
  ...
  - Owners always remain liable for payment processing/card fees and any add-ons (guest screening, damage protection); the offer waives FG commission for the first month only.
- **Pricing-surface fine print:** every plan is subject to **credit-card processing fees on guest payments, deducted from each guest payment at cost** (FG net-zero); currency conversion is likewise passed through at cost. State this clearly on every pricing surface (the marketing site pricing note carries this).
...
### Rental Income &amp; Tax Payment Accounting (CORE DIFFERENTIATOR)
The hardest part of cross-border renting, turned into a built-in feature. No competitor (Airbnb, OwnerRez, Lodgify, Hospitable, Houfy) offers owner-facing rental-income + cross-border tax accounting. We also **provide documents for tax reporting**.
...
7. **Rental Income &amp; Tax Payment Accounting** — see the dedicated section above; the standout cross-border differentiator (includes documents for tax reporting).
