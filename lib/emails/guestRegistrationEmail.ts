/**
 * HTML email template for guest account registration confirmation —
 * Familiar Guest / Tidewater style.
 *
 * Brand-level (not property- or owner-specific): confirms a successful
 * famguest.com account registration and explains the portal's role — one
 * account works across any Familiar Guest booking, and the portal is where the
 * guest messages owners and manages booking details. Ported from the provided
 * guestRegistrationEmail.js (2026-06-17 update); HTML unchanged.
 */
 ...
 * famguest.com guest account registration.
 */
export function buildGuestRegistrationEmail(fields: GuestRegistrationFields): {
  subject: string;
  html: string;
} {
  const required: (keyof GuestRegistrationFields)[] = [
    "guestName",
    "guestEmail",
    "portalUrl",
  ];
  ...
  if (missing.length > 0) {
    throw new Error(
      `buildGuestRegistrationEmail: missing required field(s): ${missing.join(", ")}`
    );
  }
  ...
  const subject = `You're all set — your Familiar Guest account is ready`;
          ...
          <tr>
            <td style="padding: 36px 32px 8px;">
              <h1 style="margin: 0 0 12px; font-family: Georgia, 'Times New Roman', serif; font-size: 26px; font-weight: 700; color: ${COLORS.ink}; line-height: 1.3;">
                You're registered, ${guestName}
              </h1>
              <p style="margin: 0; font-family: Inter, Helvetica, Arial, sans-serif; font-size: 15px; color: ${COLORS.body}; line-height: 1.6;">
                Your Familiar Guest account has been created successfully. This one account works for any booking you make on famguest.com, no matter the property.
              </p>
            </td>
          </tr>
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="padding: 6px 0; font-family: Inter, Helvetica, Arial, sans-serif; font-size: 14px; color: ${COLORS.body}; line-height: 1.6;">
                    • Message your host directly about any stay
                  </td>
                </tr>
                <tr>
                  <td style="padding: 6px 0; font-family: Inter, Helvetica, Arial, sans-serif; font-size: 14px; color: ${COLORS.body}; line-height: 1.6;">
                    • View or update your booking details
                  </td>
                </tr>
                <tr>
                  <td style="padding: 6px 0; font-family: Inter, Helvetica, Arial, sans-serif; font-size: 14px; color: ${COLORS.body}; line-height: 1.6;">
                    • Access check-in instructions and trip info in one place
                  </td>
                </tr>
              </table>
            </td>
